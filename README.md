...
### Mode-line or popup colors have poor contrast

This is usually caused by a **palette-biased theme** — one where most colors
belong to the same hue family (common with themes generated by tools like the
Aether theme builder). The themer selects UI background colors from the palette
itself, so if every palette color is a similar shade of teal or warm earth tone,
there is no algorithmically safe choice that will contrast well with the text.

See [docs/PALETTE-DESIGN.md](docs/PALETTE-DESIGN.md) for a full explanation of
what makes a palette work well, why there is no generic fix for biased palettes,
and how to work around it with per-face Doom config overrides while a proper
per-theme override mechanism is developed.
